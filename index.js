        ...
        $(Slider.SELECTORS.select_button).on("click",function() {
            ...
            if (selectIndex > Slider.GLOBAL.index) {
                ...
                for (let i = 0; i < subIndex; i++) {
                    ...
                    $(Slider.SELECTORS.select_button + `[index=${Slider.GLOBAL.index}]`).removeClass("active");
                    Slider.GLOBAL.index = Slider.GLOBAL.index >= Number(Slider.GLOBAL.imgCount)-1 ? 0 : Number(Slider.GLOBAL.index)+1;
            ...
            if (selectIndex < Slider.GLOBAL.index) {
                ...
                for (let i = 0; i < subIndex; i++) {
                    ...
                    $(Slider.SELECTORS.select_button + `[index=${Slider.GLOBAL.index}]`).removeClass("active");
                    Slider.GLOBAL.index = Slider.GLOBAL.index <= 0 ? Number(Slider.GLOBAL.imgCount)-1 : Number(Slider.GLOBAL.index)-1;
                    $(Slider.SELECTORS.select_button + `[index=${Slider.GLOBAL.index}]`).addClass("active");
                }
            }
        })
